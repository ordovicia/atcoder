...
fn main() {
    let train_normal = parse_line::<i32>();
    let train_free = parse_line::<i32>();
    let bus_normal = parse_line::<i32>();
    let bus_free = parse_line::<i32>();

    let fee_min = cmp::min(train_normal, train_free) + cmp::min(bus_normal, bus_free);
    println!("{}", fee_min);
}
