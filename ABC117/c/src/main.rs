...
fn main() {
    input!(stones_num: usize, pos_num: usize, pos: [i32; pos_num]);

    if stones_num >= pos_num {
        println!("{}", 0);
        return;
    }
    ...
    let mut intervals = pos.windows(2).map(|w| w[1] - w[0]).collect::<Vec<_>>();
    intervals.sort_by(|d1, d2| d2.cmp(&d1));

    let ans = intervals.iter().skip(stones_num - 1).sum::<i32>();
    println!("{}", ans);
}
