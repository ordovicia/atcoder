...
fn main() {
    input!(
        ...
        temple_num: usize,
        query_num: usize,
        shrines: [i64; shrine_num],
        temples: [i64; temple_num],
        queries: [i64; query_num]
    );

    for q in queries {
        let (shrine_w, shrine_e) = near_poss(&shrines, q);
        let (temple_w, temple_e) = near_poss(&temples, q);

        let ans = cmp::min(
            cmp::min(
                cmp::max(q - shrine_w, q - temple_w),
                cmp::max(shrine_e - q, temple_e - q),
            ),
            cmp::min(
                cmp::min(q - shrine_w, temple_e - q) + (temple_e - shrine_w),
                cmp::min(shrine_e - q, q - temple_w) + (shrine_e - temple_w),
            ),
        );
...
fn near_poss(poss: &[i64], q: i64) -> (i64, i64) {
    let idx = match poss.binary_search(&q) {
        Ok(idx) => idx,
        Err(idx) => idx,
    };

    let west = if idx == 0 {
        std::i64::MIN / 3 // avoid overflow (NOTE: 2^63 ~ 10^19)
    } else {
        poss[idx - 1]
    };

    let east = if idx == poss.len() {
        std::i64::MAX / 3
    } else {
        poss[idx]
    };
