...
fn main() {
    ...
    let mut remain_ingreds = parse_word::<i32>();

    let mut min_ingred = 1000;
    for _ in 0..n {
        let ingred = parse_line::<i32>();
        min_ingred = cmp::min(min_ingred, ingred);
        remain_ingreds -= ingred;
    }

    let donut_cnt = n + (remain_ingreds / min_ingred) as usize;
    println!("{}", donut_cnt);
}
