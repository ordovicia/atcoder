...
fn main() {
    inputln!(store_num: usize, drink_num: u64);
    ...
    let mut stores = (0..store_num)
        .map(|_| {
            inputln!(p: u64, s: u64);
            Store { price: p, stock: s }
        })
        .collect::<Vec<_>>();
    stores.sort_by_key(|s| s.price);

    let mut price_sum = 0;
    let mut bought_sum = 0;
    for s in stores {
        let need = drink_num - bought_sum;
        if need <= s.stock {
            price_sum += s.price * need;
            break;
        } else {
            price_sum += s.price * s.stock;
            bought_sum += s.stock;
        }
    }
