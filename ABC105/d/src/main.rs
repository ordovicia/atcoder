...
fn main() {
    input!(
        box_num: usize,
        child_num: usize,
        candy_num: [usize; box_num]
    );

    // prefix_sum[0] = 0,
    // prefix_sum[i] = candy_num[0] + ... + candy_num[i - 1]
    let mut prefix_sum = 0;

    // Number of i s.t. prefix_sum[i] % child_num == key
    let mut mod_cnt = HashMap::new();
    mod_cnt.insert(0, 1);

    let mut ans = 0_usize;
    for candy in candy_num {
        prefix_sum += candy;
        let entry = mod_cnt.entry(prefix_sum % child_num).or_insert(0);
        ans += *entry;
        *entry += 1;
    }
